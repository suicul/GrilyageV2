...
export default function HomePage() {
  ...
  async function submitOrder() {
    ...
    try {
      const body = {
        items: cart.map((item) => ({ productId: item.name, qty: item.qty })),
        deliveryMode,
        paymentMethod: 'CASH',
      ...
      <section className="hero">
        ...
        <header className="header">
          ...
          <div className="header-right">
            ...
            {mounted && (
              <button className="circle-btn" type="button" onClick={() => setAuthModalOpen(true)} style={{ fontSize: 14, minWidth: 46 }}>
                {user ? '👤' : 'Войти'}
              </button>
            )}
          </div>
